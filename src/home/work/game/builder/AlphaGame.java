...
public final class AlphaGame extends BaseGame {
    UI console = new ChessConsole();

    public AlphaGame(String player) {
        super(player);
    }
    ...
    public void printBoard() {
        console.printMessage("Turn " + nowPlayerColor());

        console.setRow("");
        console.setRow("Player 2(Black)");
        console.setRow("");
        console.setRow("\t0\t1\t2\t3\t4\t5\t6\t7");
        for (int i = 7; i > -1; i--) {
            console.setLabel(i + "\t", i);
            for (int j = 0; j < 8; j++) {
                if (this.isEmptyCell(i, j)) {
                    console.setCell(".." + "\t", i, j);
                } else {
                    console.setCell(this.getPieceOnCell(i, j).getSymbol() + this.getPieceOnCell(i, j).getColor().substring(0, 1).toLowerCase() + "\t", i, j);
                }
            }
            console.setRow("");
            console.setRow("");
        }
        console.setRow("Player 1(White)");
    }
